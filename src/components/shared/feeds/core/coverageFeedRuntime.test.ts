...
import { areCoverageItemsEqual } from './utils';
import { normalizePressCoverage } from '@shared/feeds/press/pressCoverage';
import { normalizeSpeakingEvents } from '@shared/feeds/speaking/speakingEvents';
...
const buildFeedFixture = (
  ...
  document.body.innerHTML = `
    <section
      ...
      data-coverage-items='${JSON.stringify(items)}'
    >
...
describe('coverageFeedRuntime', () => {
  ...
  it('updates when an existing item changes non-key content', async () => {
    const localItems = normalizePressCoverage([
      {
        date: '2024-01-10',
        title: 'Local Item',
    ...
    const controller = hydrateCoverageFeed(root, {
      autoRemoteCheck: false,
      fetcher: createFetcher(remotePayload),
    });

    const didUpdate = await controller?.refreshFromRemote();
    expect(didUpdate).toBe(true);
    expect(root.getAttribute('data-coverage-items')).toContain(
      'Updated Publication',
    );
  });

  it('does not update when only remote ordering differs', async () => {
    const localItems = normalizeSpeakingEvents([
      {
        date: '2025-03-10',
        title: 'New Event',
    ...
    const { root } = buildFeedFixture('speaking', localItems);
    const controller = hydrateCoverageFeed(root, {
      autoRemoteCheck: false,
      fetcher: createFetcher(remotePayload),
    });
    ...
    expect(
      areCoverageItemsEqual(
        localItems,
        normalizeSpeakingEvents(remotePayload),
      ),
    ).toBe(true);
  });
});
