...
import { normalizeSpeakingEvents } from '../speaking/speakingEvents';
...
const readFeedState = (root: HTMLElement): CoverageFeedState | null => {
  ...
  const rawItems = root.getAttribute('data-coverage-items') || '[]';
  ...
  const items = normalizeCoverageByKind(kindValue, parsedItems);
...
const applyUpdatedItems = (
  ...
  elements.root.setAttribute('data-coverage-items', JSON.stringify(nextItems));
};
...
export const hydrateCoverageFeed = (
  ...
  const refreshFromRemote = async (): Promise<boolean> => {
    ...
    if (areCoverageItemsEqual(state.items, nextItems)) {
      return false;
    }
