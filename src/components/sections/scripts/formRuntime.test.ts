...
import { initFormRuntime } from '@sections/scripts/formRuntime';
import {
...
describe('formRuntime modal behavior', () => {
  ...
  it('shows success modal and resets form in mock mode', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(title).toHaveTextContent('Success. Message sent (yellow checkmark)');
  ...
  it('shows failure modal in mock mode when trigger text is used', async () => {
    ...
    await vi.waitFor(() => {
      expect(title).toHaveTextContent('Message not sent (red x emoji)');
